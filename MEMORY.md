...
- **YouTube字幕取得** (`scripts/youtube-subtitle.sh`)：2026-02-01
  ...
  - 動画の内容を要約して活用可能
- **Notion連携システム** (2026-02-01)
  - クライアント別マッピング: `config/client-notion-mapping.json`
  - 自動書き込み: `scripts/notion-client-append.js`
  - クライアント同期: `scripts/notion-sync-clients.js`
  - データベース照会: `scripts/notion-db-query.js`

## Notion連携の仕組み
- **クライアント管理DB**: `250fa1d8-cd3d-461c-a9c2-854b2e99804d`
- **Discord ↔ Notion マッピング**（全6クライアント）
  - 詳細は `config/client-notion-mapping.json` に保存（トークン節約）
- **使い方**: `node scripts/notion-client-append.js <channelId> <content>`
  - チャンネルIDから自動で正しいクライアントページに保存
- **確認**: `node scripts/notion-sync-clients.js` でクライアント一覧表示
- **01_楽楽関連チャンネル** (ID: 1468109213443293269)
  - 数値管理・請求書進捗管理用
  - フロムアイズ案件の場合のみ稟議番号を確認

## 案件ステータス管理システム（2026-02-03）
- **目的**: 各クライアントの案件進捗をNotionで一元管理
- **ワークフロー**: 案件作成 → 見積もり → 作業 → 検収 → 請求 → 完了（全12ステップ）
- **使い方**: クライアントチャンネルで「見積もり送った」等のキーワードで自動記録
- **スクリプト**: `scripts/project-status.js`
- **詳細**: `docs/PROJECT_STATUS_SYSTEM.md`

## クライアント情報管理ルール（2026-02-01制定）
- **基本：クライアント横断禁止** — 各クライアントの情報は他クライアントで使わない
- **保存先：Notion直接記録** — メモリファイルには書かない（トークン節約＆情報漏洩防止）
- **取得：必要時のみ** — Notion APIで該当クライアントの情報だけ取得
